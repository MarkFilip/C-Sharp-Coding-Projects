...
namespace CarInsurance.Controllers
{
    public class InsureeController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,FirstName,LastName,EmailAddress,DateOfBirth,CarYear,CarMake,CarModel,DUI,SpeedingTickets,CoverageType,Quote")] Table table)
        {
            ...
            {
                TimeSpan age = DateTime.Now - table.DateOfBirth;
                if (Convert.ToInt32(age.TotalDays)/365 <= 18)
				{
                    table.Quote += 100;
				}
                else if (Convert.ToInt32(age.TotalDays)/365 >= 19 && Convert.ToInt32(age.TotalDays)/365 <=25)
				{
                    table.Quote += 50;
				}
                else
                ...
                if (table.CoverageType == "Full")
				{
                    table.Quote = table.Quote * 1.5m;
				}
                db.Tables.Add(table);
                db.SaveChanges();
                return RedirectToAction("QuoteDisplay", new { id = table.Id });
            }

            return View(table);
        }

        public ActionResult QuoteDisplay(int? id)
		{
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
